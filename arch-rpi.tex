...
\item (\lstinline{systemctl} seems to have replaced \lstinline{initscripts}, so ignore any advice that says you need to look at your \lstinline{/etc/rc.conf} file, which you probably don't even have!)

Check \lstinline{systemctl} to see if an SSH d\ae mon is running\footnote{Trust me, you don't want to leave off the \lstinline{| grep -i 'ssh'}---it's a bit much to read through.  Also, the \lstinline{-i} isn't strictly necessary in this case, but it gives you a case-insensitive search, which is nice.}:
\begin{lstlisting}
