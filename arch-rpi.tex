...
\item Edit the newly-copied \lstinline{<//<profile>//>} with \lstinline{nano} or \lstinline{vi}\footnote{There may be other editors included in Arch, but \lstinline{# which ed} and \lstinline{# which emacs} both failed, so I'm guessing our choices are very limited.}:
\begin{lstlisting}
